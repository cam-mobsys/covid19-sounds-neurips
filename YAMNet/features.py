...
def pad_waveform(waveform, params):
    ...
    min_waveform_seconds = params.patch_window_seconds + params.stft_window_seconds - params.stft_hop_seconds
    ...
    num_hops_after_first_patch = tf.cast(
        tf.math.ceil(tf.cast(num_samples_after_first_patch, tf.float32) / tf.cast(hop_samples, tf.float32)), tf.int32
    )
    num_padding_samples += hop_samples * num_hops_after_first_patch - num_samples_after_first_patch

    padded_waveform = tf.pad(waveform, [[0, num_padding_samples]], mode="CONSTANT", constant_values=0.0)
...
def _tflite_stft_magnitude(signal, frame_length, frame_step, fft_length):
    ...
    def _hann_window():
        return tf.reshape(
            tf.constant(
                (0.5 - 0.5 * np.cos(2 * np.pi * np.arange(0, 1.0, 1.0 / frame_length))).astype(np.float32),
                name="hann_window",
            ),
            [1, frame_length],
        )
    ...
    def _rdft(framed_signal, fft_length):
        ...
        real_dft_matrix = tf.constant(
            np.real(complex_dft_matrix_kept_values).astype(np.float32), name="real_dft_matrix"
        )
        imag_dft_matrix = tf.constant(
            np.imag(complex_dft_matrix_kept_values).astype(np.float32), name="imaginary_dft_matrix"
        )
