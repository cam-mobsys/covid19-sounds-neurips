...
class YAMNetTest(tf.test.TestCase):
    ...
    def testZeros(self):
        self.clip_test(waveform=np.zeros((int(3 * YAMNetTest._params.sample_rate),)), expected_class_name="Silence")

    def testRandom(self):
        np.random.seed(51773)  # Ensure repeatability.
        self.clip_test(
            waveform=np.random.uniform(-1.0, +1.0, (int(3 * YAMNetTest._params.sample_rate),)),
            expected_class_name="White noise",
        )

    def testSine(self):
        self.clip_test(
            waveform=np.sin(2 * np.pi * 440 * np.arange(0, 3, 1 / YAMNetTest._params.sample_rate)),
