...
def _batch_norm(name, params):
    def _bn_layer(layer_input):
        return layers.BatchNormalization(
            name=name, center=params.batchnorm_center, scale=params.batchnorm_scale, epsilon=params.batchnorm_epsilon
        )(layer_input)
...
def yamnet(features, params):
    ...
    net = layers.Reshape(
        (params.patch_frames, params.patch_bands, 1), input_shape=(params.patch_frames, params.patch_bands)
    )(features)
...
def yamnet_frames_model(params):
    ...
    frames_model = Model(name="yamnet_frames", inputs=waveform, outputs=[predictions, embeddings, log_mel_spectrogram])
    return frames_model
