...
assert tf.version.VERSION >= "2.0.0", "Need at least TF 2.0, you have TF v{}".format(tf.version.VERSION)
...
def check_model(model_fn, class_map_path, params):
    ...
    def clip_test(waveform, expected_class_name, top_n=10):
        ...
        assert (
            expected_class_name in top_n_class_names
        ), "Did not find expected class {} in top {} predictions: {}".format(
            expected_class_name, top_n, top_n_predictions
        )

    clip_test(waveform=np.zeros((int(3 * params.sample_rate),), dtype=np.float32), expected_class_name="Silence")

    np.random.seed(51773)  # Ensure repeatability.
    clip_test(
        waveform=np.random.uniform(-1.0, +1.0, (int(3 * params.sample_rate),)).astype(np.float32),
        expected_class_name="White noise",
    )

    clip_test(
        waveform=np.sin(2 * np.pi * 440 * np.arange(0, 3, 1 / params.sample_rate), dtype=np.float32),
...
def make_tf2_export(weights_path, export_dir):
    if os.path.exists(export_dir):
        log("TF2 export already exists in {}, skipping TF2 export".format(export_dir))
        return
...
def make_tfjs_export(tflite_saved_model_dir, export_dir):
    if os.path.exists(export_dir):
        log("TF-JS export already exists in {}, skipping TF-JS export".format(export_dir))
        return

    # Make a TF-JS model from the TF-Lite SavedModel export.
    log("Making TF-JS model ...")
    os.makedirs(export_dir)
    tfjs_saved_model_converter.convert_tf_saved_model(tflite_saved_model_dir, export_dir)
    log("Done")
